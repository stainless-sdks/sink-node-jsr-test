...
export class MixedParams extends APIResource {
   ...
   */
  bodyWithTopLevelOneOfAndPath(pathParam: string, body: MixedParamBodyWithTopLevelOneOfAndPathParams, options?: Core.RequestOptions): Core.APIPromise<void> {
    return this._client.post(`/mixed_params/body_with_top_level_one_of_and_path/${pathParam}`, { body, ...options, headers: { Accept: '*/*', ...options?.headers } });
  }

  /**
   * Endpoint with a `requestBody` that defines both query and body params
   */
  queryAndBody(params?: MixedParamQueryAndBodyParams, options?: Core.RequestOptions): Core.APIPromise<Shared.BasicSharedModelObject>
  queryAndBody(options?: Core.RequestOptions): Core.APIPromise<Shared.BasicSharedModelObject>
  queryAndBody(params: MixedParamQueryAndBodyParams | Core.RequestOptions = {}, options?: Core.RequestOptions): Core.APIPromise<Shared.BasicSharedModelObject> {
   ...
   */
  queryBodyAndPath(pathParam: string, params?: MixedParamQueryBodyAndPathParams, options?: Core.RequestOptions): Core.APIPromise<Shared.BasicSharedModelObject>
  queryBodyAndPath(pathParam: string, options?: Core.RequestOptions): Core.APIPromise<Shared.BasicSharedModelObject>
  queryBodyAndPath(pathParam: string, params: MixedParamQueryBodyAndPathParams | Core.RequestOptions = {}, options?: Core.RequestOptions): Core.APIPromise<Shared.BasicSharedModelObject> {
    if (isRequestOptions(params)) {
      return this.queryBodyAndPath(pathParam, {}, params);
    }
    const { query_param, ...body } = params;
    return this._client.post(`/mixed_params/query_body_and_path/${pathParam}`, { query: { query_param }, body, ...options });
  }
}

export type MixedParamBodyWithTopLevelOneOfAndPathParams = MixedParamBodyWithTopLevelOneOfAndPathParams.ObjectWithRequiredEnum | MixedParamBodyWithTopLevelOneOfAndPathParams.BasicSharedModelObject

...
export class SharedQueryParams extends APIResource {
  ...
  retrieve(query: SharedQueryParamRetrieveParams | Core.RequestOptions = {}, options?: Core.RequestOptions): Core.APIPromise<string> {
    if (isRequestOptions(query)) {
      return this.retrieve({}, query);
    }
    return this._client.get('/shared-query-params', { query, ...options, headers: { Accept: 'application/json', ...options?.headers } });
  }
  ...
  del(params: SharedQueryParamDelParams | Core.RequestOptions = {}, options?: Core.RequestOptions): Core.APIPromise<string> {
    if (isRequestOptions(params)) {
      return this.del({}, params);
    }
    const { get1, shared1, shared2 } = params;
    return this._client.delete('/shared-query-params', { query: { get1, shared1, shared2 }, ...options, headers: { Accept: 'application/json', ...options?.headers } });
  }
}

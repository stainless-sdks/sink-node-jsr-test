...
describe('resource keepThisResource', () => {
  test('keepThisMethod', async () => {
    const responsePromise = client.decoratorTests.keepThisResource.keepThisMethod();
  ...
  test('keepThisMethod: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.decoratorTests.keepThisResource.keepThisMethod({ path: '/_stainless_unknown_path' }))
      .rejects

...
describe('resource languages', () => {
  test('skippedForPython', async () => {
    const responsePromise = client.decoratorTests.languages.skippedForPython();
  ...
  test('skippedForPython: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.decoratorTests.languages.skippedForPython({ path: '/_stainless_unknown_path' }))
      .rejects

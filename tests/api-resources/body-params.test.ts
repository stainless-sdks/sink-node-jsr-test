...
describe('resource body_params', () => {
  ...
  test('top_level_allOf_nested_object: required and optional params', async () => {
    const response = await sink.bodyParams.topLevelAllOfNestedObject({ kind: 'VIRTUAL',nested_obj: { is_foo: true } })
  })
  ...
  test('union_overlapping_prop: request options instead of params are passed correctly', async () => {
    ...
    await expect(sink.bodyParams.unionOverlappingProp({ path: '/_stainless_unknown_path' }))
    .rejects
  ...
  test('union_overlapping_prop: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(sink.bodyParams.unionOverlappingProp({ foo: 'string' }, { path: '/_stainless_unknown_path' }))
    .rejects
    .toThrow(Sink.NotFoundError)
  })

...
describe('resource envelopes', () => {
  ...
  test('explicit: request options instead of params are passed correctly', async () => {
    ...
    await expect(sink.envelopes.explicit({ path: '/_stainless_unknown_path' }))
    .rejects
  ...
  test('implicit: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(sink.envelopes.implicit({ path: '/_stainless_unknown_path' }))
    .rejects
    .toThrow(Sink.NotFoundError)
  })

  test('inline_response', async () => {
    const response = await sink.envelopes.inlineResponse()

  })

  test('inline_response: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(sink.envelopes.inlineResponse({ path: '/_stainless_unknown_path' }))
    .rejects
    .toThrow(Sink.NotFoundError)
  })
})
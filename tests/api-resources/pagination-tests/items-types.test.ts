...
describe('resource itemsTypes', () => {
  ...
  test('listUnknown: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.paginationTests.itemsTypes.listUnknown({ path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('listUnknown: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.paginationTests.itemsTypes.listUnknown({ page: 0, page_size: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Sink.NotFoundError);
  });
});

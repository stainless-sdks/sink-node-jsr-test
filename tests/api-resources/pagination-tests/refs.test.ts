...
describe('resource refs', () => {
  ...
  test('nestedObjectRef: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.paginationTests.refs.nestedObjectRef({ path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('nestedObjectRef: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.paginationTests.refs.nestedObjectRef({ cursor: 'cursor', limit: 0, object_param: { foo: 'foo' } }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Sink.NotFoundError);
  });
  ...
  test('withSharedModelRef: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.paginationTests.refs.withSharedModelRef({ path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Sink.NotFoundError);
  });
  ...
  test('withSharedModelRef: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.paginationTests.refs.withSharedModelRef({ cursor: 'cursor', limit: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Sink.NotFoundError);
  });
});

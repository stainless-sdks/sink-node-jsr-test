...
describe('resource schemaTypes', () => {
  ...
  test('allofs: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.paginationTests.schemaTypes.allofs({ path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('allofs: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.paginationTests.schemaTypes.allofs({ cursor: 'cursor', limit: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Sink.NotFoundError);
  });
  ...
  test('unions: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.paginationTests.schemaTypes.unions({ path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Sink.NotFoundError);
  });
  ...
  test('unions: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.paginationTests.schemaTypes.unions({ cursor: 'cursor', limit: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Sink.NotFoundError);
  });
});

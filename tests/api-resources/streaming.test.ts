...
describe('resource streaming', () => {
  ...
  test('nestedParams: required and optional params', async () => {
    const response = await client.streaming.nestedParams({ model: 'model', prompt: 'prompt', parent_object: { array_prop: [{ from_array_items: true }, { from_array_items: true }, { from_array_items: true }], child_prop: { from_object: 'from_object' } }, stream: false });
  });
  ...
  test('queryParamDiscriminator: required and optional params', async () => {
    const response = await client.streaming.queryParamDiscriminator({ prompt: 'prompt', should_stream: false });
  });
});

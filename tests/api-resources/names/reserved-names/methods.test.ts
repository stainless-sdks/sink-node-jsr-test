...
describe('resource methods', () => {
  test('export', async () => {
    const responsePromise = client.names.reservedNames.methods.export('class');
  ...
  test('export: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.names.reservedNames.methods.export('class', { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('export: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.names.reservedNames.methods.export('class', { let: 'let', const: 'const' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Sink.NotFoundError);
  });
});

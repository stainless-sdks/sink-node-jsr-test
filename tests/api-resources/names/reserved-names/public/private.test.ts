...
describe('resource private', () => {
  test('private', async () => {
    const responsePromise = client.names.reservedNames.public.private.private();
  ...
  test('private: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.names.reservedNames.public.private.private({ path: '/_stainless_unknown_path' }))
      .rejects

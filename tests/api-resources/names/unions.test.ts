...
describe('resource unions', () => {
  ...
  test('discriminated: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.names.unions.discriminated({ path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('variantsObjectWithUnionProperties: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.names.unions.variantsObjectWithUnionProperties({ path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Sink.NotFoundError);
  });
  ...
  test('variantsSinglePropObjects: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.names.unions.variantsSinglePropObjects({ path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Sink.NotFoundError);
  });
});

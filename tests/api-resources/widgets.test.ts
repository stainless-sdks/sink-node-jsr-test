...
describe('resource widgets', () => {
  test('retrieveWithFilter', async () => {
    const responsePromise = client.widgets.retrieveWithFilter(0, 'available');
  ...
  test('retrieveWithFilter: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.widgets.retrieveWithFilter(0, 'available', { path: '/_stainless_unknown_path' }))
      .rejects

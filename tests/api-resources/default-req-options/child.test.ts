...
describe('resource child', () => {
  test('exampleMethod', async () => {
    const responsePromise = client.defaultReqOptions.child.exampleMethod();
  ...
  test('exampleMethod: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.defaultReqOptions.child.exampleMethod({ path: '/_stainless_unknown_path' }))
      .rejects

...
describe('resource sta_569', () => {
  ...
  test('one_entry_with_null: request options instead of params are passed correctly', async () => {
    ...
    await expect(sink.sta_569.oneEntryWithNull({ path: '/_stainless_unknown_path' })).rejects.toThrow(
      Sink.NotFoundError,
    );
  });
  ...
  test('one_entry_with_null: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      sink.sta_569.oneEntryWithNull({ enable_debug_logging: true }, { path: '/_stainless_unknown_path' }),

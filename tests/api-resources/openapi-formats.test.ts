...
describe('resource openAPIFormats', () => {
  ...
  test('arrayTypeOneEntryWithNull: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.openAPIFormats.arrayTypeOneEntryWithNull({ path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('arrayTypeOneEntryWithNull: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.openAPIFormats.arrayTypeOneEntryWithNull({ enable_debug_logging: true }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Sink.NotFoundError);
  });
});

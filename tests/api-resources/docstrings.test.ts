...
describe('resource docstrings', () => {
  ...
  test('descriptionContainsJsDoc: request options instead of params are passed correctly', async () => {
    ...
    await expect(
      client.docstrings.descriptionContainsJsDoc({ path: '/_stainless_unknown_path' }),
  ...
  test('descriptionContainsJsDocEnd: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.docstrings.descriptionContainsJsDocEnd({ path: '/_stainless_unknown_path' }),
    ).rejects.toThrow(Sink.NotFoundError);
  });
  ...
  test('leadingDoubleQuote: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.docstrings.leadingDoubleQuote({ path: '/_stainless_unknown_path' })).rejects.toThrow(
      Sink.NotFoundError,
    );
  });
  ...
  test('trailingDoubleQuote: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.docstrings.trailingDoubleQuote({ path: '/_stainless_unknown_path' })).rejects.toThrow(
      Sink.NotFoundError,
    );
  });
});
